...
path <- paste(urld, "acc=GSE267554", "file=GSE267554_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
write.table(tbl, file="GSE267554_counts.tsv", sep="\t", row.names=FALSE, quote=FALSE)
write.table(annot, file="Human.GRCh38.p13.annot.tsv", sep="\t", row.names=FALSE, quote=FALSE)


# sample selection
gsms <- "000XXXXXXXXXX111"
...
gs <- factor(sml)
groups <- make.names(c("ctrl","12 hours"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE267554 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE267554 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE267554", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
